...
def show():
    ...
    # Remove colored dots from expander headers
    st.markdown("""
    <style>
    ...
    with st.expander("Measure 1 — Missing Data Handling", expanded=True):
        ...
        if missing_summary.empty:
            ...
            st.markdown(
                ...
                f"across <b>{len(missing_summary)}</b> columns.</p>",
                unsafe_allow_html=True
            )
            ...
            if st.button("Apply Missing Data Fix", key="apply_missing"):
                ...
                fixed = before_nulls - after_nulls
                store_message("missing", "success",
                    f"✅ Strategy '{strategy}' applied. Fixed {fixed:,} missing values. Remaining: {after_nulls:,}")
                st.rerun()

    # ═════════════════════════════════════════════════════════════════════════
    # MEASURE 2 — OUTLIER HANDLING
    # ═════════════════════════════════════════════════════════════════════════
    with st.expander("Measure 2 — Outlier Detection & Handling", expanded=False):
        ...
        if not outlier_counts:
            ...
            st.markdown(
                ...
                f"<b>{len(outlier_counts)}</b> columns.</p>",
                unsafe_allow_html=True
            )
            ...
            if stats:
                ...
                fig.update_layout(
                    title=dict(
                        text="Outliers Detected per Column",
                        x=0, font=dict(size=14, color="#1a5276")
                    ),
                    xaxis=dict(
                        title="Number of Outlier Values",
            ...
            if st.button("Apply Outlier Fix", key="apply_outliers"):
                ...
                if action == "flag":
                    ...
                    msg = f"✅ Flagged outliers — added {len(flag_cols)} flag column(s): {', '.join(flag_cols)}. Data values unchanged."
                elif action == "remove":
                    msg = f"✅ Removed {removed_rows:,} rows containing outliers. Outliers before: {before_out:,} → after: {after_out:,}"
                elif action == "cap":
                    msg = f"✅ Capped {before_out:,} outlier values to boundary limits. Outliers remaining: {after_out:,}"
                else:
                    msg = f"✅ Replaced {before_out:,} outliers with column mean. Outliers remaining: {after_out:,}"
                store_message("outlier", "success", msg)
                st.rerun()

    # ═════════════════════════════════════════════════════════════════════════
    # MEASURE 3 — FORMAT & TYPE VALIDATION
    # ═════════════════════════════════════════════════════════════════════════
    with st.expander("Measure 3 — Format & Type Validation", expanded=False):
        ...
        st.markdown(
            ...
            "and strips whitespace from all text columns.</p>",
            unsafe_allow_html=True
        )
        ...
        if st.button("Apply Format Fixes", key="apply_format"):
            ...
            fixes_text = " | ".join(fixes)
            store_message("format", "success", f"✅ {len(fixes)} fixes applied: {fixes_text}")
            st.rerun()

    # ═════════════════════════════════════════════════════════════════════════
    # MEASURE 4 — REMOVE DUPLICATES
    # ═════════════════════════════════════════════════════════════════════════
    with st.expander("Measure 4 — Duplicate Row Removal", expanded=False):
        ...
        st.markdown(
            f"<p style='font-size:13px; color:#555;'>"
            f"Current duplicate rows: <b>{current_dups:,}</b>. "
            f"Removing duplicates ensures every transaction is counted exactly once.</p>",
            unsafe_allow_html=True
        )
    ...
    with st.expander("Measure 5 — Shipping Duration Calculation", expanded=False):
        st.markdown(
            ...
            'Shipping Duration Calculation</p>',
            unsafe_allow_html=True
        )
        ...
        st.markdown(
            "<p style='font-size:13px; color:#555;'>"
            "Adds a <b>Shipping Duration (Days)</b> column = Ship Date − Order Date. "
            "Delayed shipments often correlate with lower customer satisfaction.</p>",
            unsafe_allow_html=True
        )

        if already_added:
            st.info("ℹ️ 'Shipping Duration (Days)' column already exists in the dataset.")
        else:
            if st.button("Apply Shipping Duration", key="apply_ship"):
                ...
                if "Shipping Duration (Days)" in st.session_state["cleaned_df"].columns:
                    ...
                    store_message("ship", "success",
                        f"✅ 'Shipping Duration (Days)' column added. Average shipping time: {avg_days:.1f} days.")
                else:
    ...
    with st.expander("Measure 6 — Text Standardisation", expanded=False):
        ...
        st.markdown(
            "<p style='font-size:13px; color:#555;'>"
            "Converts Category, Sub-Category, Segment, Region, Ship Mode to Title Case. "
            "Without this, 'furniture' and 'FURNITURE' are treated as different categories.</p>",
            unsafe_allow_html=True
        )
