...
def show():
    ...
    with st.expander("Measure 1 — Missing Data Handling", expanded=True):
        st.markdown('<p style="font-size:15px; font-weight:600; color:#1a5276; margin-bottom:12px;">'
        ...
        if missing_summary.empty:
            ...
            st.markdown(f"<p style='font-size:13px; color:#555;'>Found <b>{missing_summary['Missing Count'].sum():,}</b> "
                        f"missing values across <b>{len(missing_summary)}</b> columns.</p>", unsafe_allow_html=True)
            ...
            strategy = st.selectbox("Strategy", options=["drop", "mean", "median", "mode", "customer", "ffill", "constant"],
                format_func=lambda x: {
                    "drop": "Drop rows with missing values", "mean": "Fill with column mean",
                    "median": "Fill with column median", "mode": "Fill with most frequent value",
                    "customer": "Fill from Customer ID history", "ffill": "Forward fill",
                    "constant": "Fill with 0 / 'Unknown'",
                }[x], key="missing_strategy")
            if st.button("Apply Missing Data Fix", key="apply_missing"):
                before = int(st.session_state["cleaned_df"].isnull().sum().sum())
                st.session_state["cleaned_df"] = impute_missing(st.session_state["cleaned_df"], strategy=strategy)
                after  = int(st.session_state["cleaned_df"].isnull().sum().sum())
                store_message("missing", "success", f"✅ Strategy '{strategy}' applied. Fixed {before-after:,} missing values. Remaining: {after:,}")
                st.rerun()
    ...
    with st.expander("Measure 2 — Outlier Detection & Handling", expanded=False):
        ...
        method = st.radio("Detection method", ["iqr", "zscore"],
            format_func=lambda x: {"iqr": "IQR — values beyond Q1−1.5×IQR or Q3+1.5×IQR",
                                    "zscore": "Z-Score — values more than 3 standard deviations from the mean"}[x],
            horizontal=True, key="outlier_method", label_visibility="collapsed")
        ...
        if not outlier_counts:
            ...
            st.markdown(f"<p style='font-size:13px; color:#555;'>Detected <b>{total_outliers:,}</b> outlier values across "
            ...
            if stats:
                ...
                fig = go.Figure()
                fig.add_trace(go.Bar(x=outlier_vals, y=cols_list, orientation="h",
                    marker=dict(color=outlier_vals, colorscale=[[0,"#f9c784"],[0.5,"#e87c5a"],[1,"#c0392b"]], showscale=False),
                    text=labels, textposition="outside", cliponaxis=False))
                fig.update_layout(
                    title=dict(text="Outliers Detected per Column", x=0, font=dict(size=14, color="#1a5276")),
                    xaxis=dict(title="Number of Outlier Values", showgrid=True, gridcolor="#f0f4f8", zeroline=False),
                    yaxis=dict(autorange="reversed", tickfont=dict(size=13)),
                    plot_bgcolor="#ffffff", paper_bgcolor="#ffffff",
                    font=dict(family="Inter", size=12),
                    margin=dict(t=50, b=40, l=20, r=120), height=280, showlegend=False)
                st.plotly_chart(fig, use_container_width=True)
            section_header("Choose Outlier Action")
            action = st.selectbox("Action", options=["flag", "cap", "remove", "mean"],
                format_func=lambda x: {
                    "flag": "Flag only — mark outlier rows with a new column",
            ...
            if st.button("Apply Outlier Fix", key="apply_outliers"):
                ...
                st.session_state["cleaned_df"] = handle_outliers(st.session_state["cleaned_df"], method=method, action=action)
    ...
    with st.expander("Measure 3 — Format & Type Validation", expanded=False):
        ...
        show_message("format")
        st.markdown("<p style='font-size:13px; color:#555;'>Fixes date formats, postal code zero-padding, "
                    "numeric type casting, and strips whitespace from all text columns.</p>", unsafe_allow_html=True)
    ...
    with st.expander("Measure 4 — Duplicate Row Removal", expanded=False):
        ...
        st.markdown(f"<p style='font-size:13px; color:#555;'>Current duplicate rows: <b>{current_dups:,}</b>.</p>",
                    unsafe_allow_html=True)
        if st.button("Apply Duplicate Removal", key="apply_dedup"):
            ...
            if removed == 0:
                ...
                store_message("dedup", "success", f"✅ Removed {removed:,} duplicate rows. Rows remaining: {len(cleaned):,}")
            st.rerun()
    ...
    with st.expander("Measure 5 — Shipping Duration Calculation", expanded=False):
        ...
        st.markdown("<p style='font-size:13px; color:#555;'>Adds a <b>Shipping Duration (Days)</b> column = Ship Date − Order Date.</p>",
                    unsafe_allow_html=True)
        if already_added:
            ...
            if st.button("Apply Shipping Duration", key="apply_ship"):
                ...
                if "Shipping Duration (Days)" in st.session_state["cleaned_df"].columns:
                    ...
                    store_message("ship", "success", f"✅ Column added. Average shipping time: {avg_days:.1f} days.")
                else:
                    store_message("ship", "warning", "⚠️ Could not calculate duration. Apply Format Fixes first.")
                st.rerun()
    ...
    with st.expander("Measure 6 — Text Standardisation", expanded=False):
        ...
        show_message("text")
        st.markdown("<p style='font-size:13px; color:#555;'>Converts Category, Sub-Category, Segment, Region, "
                    "Ship Mode to Title Case.</p>", unsafe_allow_html=True)
        if st.button("Apply Text Standardisation", key="apply_text"):
            cleaned, fixed_cols = standardise_text(st.session_state["cleaned_df"])
            st.session_state["cleaned_df"] = cleaned
            store_message("text", "success", f"✅ Standardised {len(fixed_cols)} columns: {', '.join(fixed_cols)}")
            st.rerun()
